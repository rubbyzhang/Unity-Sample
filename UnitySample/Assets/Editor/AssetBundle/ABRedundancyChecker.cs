...
class ABRedundancyChecker
{
    ...
    /// 检测的资源后缀
    /// </summary>
    private string[] ConstAssetTypePostfix = new string[]
    {
            ".meta",
    ...
    /// 冗余资源类型白名单
    /// </summary>
    public List<Type> assetTypeList = 
        new List<Type>
        {
            typeof(Material),
            typeof(Texture2D),
    ...
    /// </summary>
    public string abPath = Application.streamingAssetsPath + "/assetbundle/";
...
//    [MenuItem("AB冗余检测/AB检测")]
    ...
    public void CheckABInfo(AssetBundle ab, string abName)
    {
        ...
        Debug.Log("########################################AB Name:" + abName);

        foreach (var path in allDepen)
        {
            Debug.Log("______________dependencies path:" + path);
        }

        UnityEngine.Object[] ts = ab.LoadAllAssets();
        foreach (var obj in ts)
        {
            Debug.Log(" ______________LoadAllAssets name:" + obj.name);
        }

        string[] assetNames = ab.GetAllAssetNames();
        for (int i = 0; i < assetNames.Length; ++i)
        {
            Debug.Log("______________GetAllAssetNames :" + assetNames[i]);

            if (IsNeedCheckType(assetNames[i]))
            {
                string name = assetNames[i].Substring(assetNames[i].LastIndexOf("/") + 1);
                TryAddAssetToMap(name, assetNames[i], abName);
            }
        }
        ...
        for (int i = 0; i < allDepen.Length; ++i)
        {
            ...
            {
                Debug.Log("______________LoadAsset :" + assetNames[i]);
                TryAddAssetToMap(obj.name, allDepen[i], abName, GetObjectType(obj));
            }
        }
    }

    bool IsNeedCheckType(string path)
    {
        ...
        foreach (var fix in ConstAssetTypePostfix)
        {
            if (path.ToLower().EndsWith(fix))
            {
                return true;
            }
        }
