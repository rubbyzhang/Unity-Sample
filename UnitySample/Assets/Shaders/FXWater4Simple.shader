...
Properties {
	...
	_RippleTex("RippleTex", 2D) = "black" {}
	_RippleTile("RippleTile", Range(0, 100)) = 5
...
Subshader
{
	//TODO 不知道啥Type从Transparent修改到Opaque 会有啥影响，待定
	...
	Pass {
			...
			#pragma multi_compile RIPPLE_OFF RIPPLE_ON
			...
			#if RIPPLE_ON
				o.rippleCoords = worldSpaceVertex.xz / _RippleTile;
			#endif
			...
			half4 frag(v2f i) : SV_Target
			{
				half3 worldNormal = PerPixelNormal(_BumpMap, i.bumpCoords, VERTEX_WORLD_NORMAL, PER_PIXEL_DISPLACE);
		#ifdef RIPPLE_ON
				half4 ripple = tex2D(_RippleTex, i.rippleCoords.xy) * 2 - 1;
				worldNormal.xz += ripple.xy*(1 - ripple.z);
				worldNormal.y = 1;
				worldNormal = normalize(worldNormal);

				//worldNormal.xz = lerp(worldNormal.xz / worldNormal.y, ripple.xy / ripple.z, 0.1);
				//worldNormal.y = 1;
				//worldNormal = normalize(worldNormal);
		#endif
