...
Shader "Kvant/Grass/Surface"
{
    ...
    SubShader
    {
        ...
        void vert(inout appdata_full v, out Input data)
        {
            ...
            float4 r = tex2Dlod(_RotationTex, uv) * 2 - float4(1, 1, 1, 1);
			float4 r2 = tex2Dlod(_RotationTex2, uv) * 2 - float4(1, 1, 1, 1);

			r.x = floor((r.x + 0.005) * 100) / 100;
			r.y = floor((r.y + 0.005) * 100) / 100;
			r.z = floor((r.z + 0.005) * 100) / 100;
			r.w = floor((r.w + 0.005) * 100) / 100;

			r2.x = floor((r2.x + 0.005) * 100) / 100;
			r2.y = floor((r2.y + 0.005) * 100) / 100;
			r2.z = floor((r2.z + 0.005) * 100) / 100;
			r2.w = floor((r2.w + 0.005) * 100) / 100;
			...
			v.vertex.xyz += float3(p.x * _Extent.x * 2 - _Extent.x, 0, p.z * _Extent.y * 2 - _Extent.y);
