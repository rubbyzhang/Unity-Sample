...
fixed4 frag (Input i) : COLOR
{
	...
	//fixed4 fog = lerp(tex2D(_FogTex0, uv), tex2D(_FogTex1, uv), 1);//lerp(tex2D(_FogTex0, uv), tex2D(_FogTex1, uv), _Params.w);
	fixed4 fog = lerp(tex2D(_FogTex0, uv), tex2D(_FogTex1, uv), _Params.w);
