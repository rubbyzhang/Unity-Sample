...
Properties {
    ...
    strength("strength", Range(0.01, 1)) = 0.2
    xoffsetspeed("xoffsetspeed",Float) = 0
    yoffsetspeed("yoffsetspeed", Float) = 0
...
v2f vert(data i){
    ...
    o.uvmain = TRANSFORM_TEX(i.texnoise, _NoiseTex);   // compute the texcoords of the noise
    o.uvmask = TRANSFORM_TEX(i.texcoord, _MainTex); 
...
{   
    ...
    //half4 offsetColor1 = tex2D(_NoiseTex, i.uvmain + float2(_Time.y,0));
    //half4 offsetColor2 = tex2D(_NoiseTex, i.uvmain + float2(_Time.y,0));
    ...
    half4 offsetColor = tex2D(_NoiseTex, i.uvmain + float2(_Time.y * xoffsetspeed,_Time.y * yoffsetspeed));
    screenPos.x += ((offsetColor.r + offsetColor.r) - 1) * i.distortion * offsetColor.a;
    screenPos.y += ((offsetColor.g + offsetColor.g) - 1) * i.distortion * offsetColor.a;
    //screenPos.x += offsetColor.r * i.distortion;
    //screenPos.y += offsetColor.g * i.distortion;
   
    half4 col = tex2D( _RenderTex, screenPos );
    half4 alphacol = tex2D( _MainTex, i.uvmask);
    col.a = (alphacol.r+alphacol.g+alphacol.b)/3 * _TintColor.a;
	return col;
}
