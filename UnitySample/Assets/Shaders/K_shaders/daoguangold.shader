...
Shader "Shader Forge/daoguang_rongjie_reimport" {
    Properties {
		...
		[MaterialToggle] _distortEnable("distortEnable", Float) = 0
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float3 normalLocal = lerp(float3(0,0,1),float3(_Diffuse_var.a,_Diffuse_var.a,_Diffuse_var.a),_4_*_Diffuse_var.a);
                float3 normalDirection = normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
                float2 sceneUVs = float2(1,grabSign)*i.screenPos.xy*0.5+0.5 + ((_Diffuse_var.rgb.rg*i.vertexColor.a)*(_4_*_Diffuse_var.a*0.1));
